...
# Ensure standard locale for string processing (prevents awk/grep locale issues)
export LC_ALL=C
...
# Prevents rapid toggling of fan speeds when temperature fluctuates near a threshold.
readonly HYST=4

# Fan Speeds (Argon Protocol: 0-100 Hexadecimal)
readonly SPEED_OFF="0x00"
readonly SPEED_QUIET="0x0A"  # 10%
readonly SPEED_LOW="0x19"    # 25%
readonly SPEED_MED="0x37"    # 55%
readonly SPEED_HIGH="0x64"   # 100%
...
#              Replaces $SECONDS which is not available in the Ash shell.
# Returns: Integer (seconds)
get_uptime() {
    local up
    if [ -r /proc/uptime ]; then
        read -r up _ < /proc/uptime
        echo "${up%%.*}" # Strip decimal precision
    else
        # Fallback if /proc is unreadable
        date +%s
    fi
}

# Function: log_info
# Description: Logs informational messages to syslog.
log_info() { 
    logger -t argon_daemon -p daemon.notice "[INFO] $1"
}

# Function: log_err
# Description: Logs error messages to syslog.
log_err() { 
...
# Description: Implements atomic locking mechanism using mkdir.
...
# Returns: Path to temp file or error code 1.
find_thermal_source() {
    local zone
    local type_val
    local found_path=""

    # Scan thermal zones for known CPU/SoC types
    ...
    # Fallback default if no specific label matches
...
# Returns: Bus number or error code 1.
find_i2c_bus() {
    local dev
    local bus_num
    local found
    ...
    for dev in /dev/i2c-*; do
        ...
        # Check for device 0x1a or UU (kernel driver occupied)
...
# Arguments: $1=Bus, $2=Register, $3=Value(Hex)
i2c_write() {
    local bus="$1"
    local reg="$2"
    local val="$3"
    
    # Suppress output, capture exit code. Use -y to disable interactive mode.
...
cleanup() {
    # Disable exit-on-error to ensure cleanup runs to completion
    set +e 
...
# Temporarily disable exit-on-error during hardware detection
# to allow for graceful error logging instead of silent crash.
set +e
...
if [ "$RES_THERMAL" -ne 0 ] || [ -z "$THERMAL_ZONE_PATH" ]; then
    log_err "CRITICAL: No valid temperature sensor found! Check kernel modules."
    # Cleanup lock manually since we are exiting before main loop
    rm -f "$PID_FILE"
    rmdir "$LOCK_DIR" 2>/dev/null
    exit 1
fi

if [ "$RES_I2C" -ne 0 ] || [ -z "$DETECTED_BUS" ]; then
